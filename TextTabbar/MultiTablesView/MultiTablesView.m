...
- (void)reloadDataCurrent:(NSUInteger)index
{

        [[self tableViewAtIndex:index] reloadData];
...
- (void)reloadData
{
    NSInteger numberOfLevels = 1;//分类数
    ...
    {//获取分类数
        ...
        NSLog(@"reloadData numberOfLevels=%d", numberOfLevels);
    }
    if (numberOfLevels > 0)
    {
        if (!self.tableViews)
